...
- **Stufe-1-Härtung Offline/Multi-User (2026-06-11) ✅:** Queue-Poison-Fix (Netzfehler zählen nicht), kein Löschen bei unerwarteten 4xx, 120-s-Timeout, Web-Locks, „hängt"-Panel (sichern/verwerfen), Client-Downscale vor Enqueue, Einfrieren serverseitig erzwungen, 48-h-Gnadenfrist für nachgereichte Medien (410 = endgültig), `abgeschlossenAm`-Migration, WAL + busy_timeout + connection_limit=1, Beitreten-Karte gegen parallele Runden, `error.tsx`, AbschlussButton blockiert bei lokalem Puffer. Smoke-Test: `scripts/smoke-stufe1.mjs` (8/8 ✅). **Entscheidung offen: Stufe 2** (offline-first Begehungsmodus: Runden-Snapshot in IndexedDB, Client-Rendering, generalisierte Op-Outbox mit Client-UUIDs, Mini-Service-Worker) — nach erstem Feldtest entscheiden.
