  ...
  try {
    ...
    if (typ === "mitteilung") {
      ...
      const fehler = await entwurfInPostfach({
        ...
        ordner: "Mitteilungen", // landet unter Entwürfe/Mitteilungen
      });
    ...
    const fehler = await entwurfInPostfach({
      ...
      anhaenge,
      ordner: "Abmahnungen BV", // landet unter Entwürfe/Abmahnungen BV
    });
    if (fehler) return { fehler, warnungen };
    return {
      ok: istEinzelAdresse(bvAdresse)
        ? `Entwurf an den Bezirksverband (${bvAdresse}) liegt im Ordner 'Entwürfe > Abmahnungen BV' — prüfen und selbst senden.`
        : "Entwurf liegt im Ordner 'Entwürfe > Abmahnungen BV' — BV-E-Mail fehlt/ungültig, Empfänger im Mail-Programm ergänzen.",
      warnungen,
    };
  } catch (e) {
    return {
