// Bericht-PDF je Parzelle bauen (aus der PDF-Route extrahiert, damit auch
// Server Actions das PDF als E-Mail-Anhang erzeugen können). Verhalten
// identisch zur bisherigen Route: Befund der Runde, sonst jüngster Befund;
// ohne Befund entsteht ein Neutral-Bericht (hatBefund=false — Mail-Aktionen
// brechen dann ab, die Anzeige-Route liefert ihn weiter aus).
...
export async function parzellenBericht(
  parzelleId: string,
  rundeId: number | null
): Promise<{ dateiname: string; pdf: Buffer; hatBefund: boolean } | null> {
  ...
  return { dateiname, pdf, hatBefund: Boolean(befund) };
}
